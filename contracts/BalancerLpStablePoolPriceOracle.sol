 ...
 */
contract BalancerLpStablePoolPriceOracle is
    OwnableUpgradeable,
    BasePriceOracle
{
    ...
    function _getUsdPrice(address _token) internal view returns (uint256) {
        ...
        if(isBoostedToken[_token] == true) {
            return IChainlinkManager(chainlinkManager).getUSDPrice(_token) * IERC4626LinearPool(_token).getRate() / 1e18;
        }

        return IChainlinkManager(chainlinkManager).getUSDPrice(_token);
    }
}
